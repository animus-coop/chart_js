  ...
  addData(label, datasets) {
    ...
    if (datasets && Array.isArray(datasets)) {
      datasets.forEach((datasetUpdate, index) => {
        const targetIndex = datasetUpdate.datasetIndex !== undefined ? datasetUpdate.datasetIndex : index;
        if (this.chart.data.datasets[targetIndex] && datasetUpdate.data !== undefined) {
          this.chart.data.datasets[targetIndex].data.push(datasetUpdate.data);
        }
      });
    }
